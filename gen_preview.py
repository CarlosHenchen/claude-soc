...
function load(){try{return JSON.parse(localStorage.getItem(LS))||freshDB()}catch(e){return freshDB()}}
...
"""
html = TEMPLATE.replace("__DATA__", data_json).replace("__ECHARTS__", echarts_js)
with open(os.path.join(HERE, "preview.html"), "w", encoding="utf-8") as f:
    f.write(html)
print("wrote preview.html", round(os.path.getsize(os.path.join(HERE, "preview.html")) / 1024), "KB")
